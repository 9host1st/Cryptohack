...
_sage_const_2 = Integer(2); _sage_const_128 = Integer(128); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_127 = Integer(127); _sage_const_16 = Integer(16)
from Crypto.Util.number import long_to_bytes as l2b
from Crypto.Util.number import bytes_to_long as b2l
def bitwiseXor(a, b):
    return bytes([(x ^ y) for x, y in zip(a, b)])

F = GF(_sage_const_2 **_sage_const_128 , names='a')
(a,) = F._first_ngens(_sage_const_1 )
R = PolynomialRing(F, names='X')
(X,) = R._first_ngens(_sage_const_1 )

def block_to_poly(block):
    global F
    f = _sage_const_0 
    for e, bit in enumerate(bin(block).replace('0b','').rjust(_sage_const_128 ,'0')):
        f += int(bit) * a**e
    return f
...
def poly_to_int(poly):
    ...
    for i, bit in enumerate(poly._vector_()):
        a |= int(bit) << (_sage_const_127  - i)
    return a

def make_bytes_to_poly(msg):
    pad_cnt = (_sage_const_16  - len(msg))%_sage_const_16 
    msg += '\x00' * pad_cnt
    return [block_to_poly(b2l(msg[i*_sage_const_16 :(i+_sage_const_1 )*_sage_const_16 ])) for i in range(len(msg)/_sage_const_16 )]
